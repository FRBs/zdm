...
def main(filenames,labels,prefix,burnin=200,version=1):
    """
    ...
    # Multiple files are independent MCMC runs that can be added together for a larger data-set
    ...
    # removes bad walkers
    Nsteps,Nwalkers,Nparams = sample.shape
    GoodWalkers=np.arange(Nwalkers)
    #badlist = np.arange(1)+38
    badlist = [11,39] #these got stuck in my run. Might not be in yours!
    print("CHECK THIS FOR YOUR SAMPLE!!!!\n Removing walkers ",badlist)
    GoodWalkers = np.delete(GoodWalkers,badlist)
    samples[0] = samples[0][:,GoodWalkers,:]
    ...
    therange=[[-1,0],[1,4],[-3,0],[1.5,2.5],[0.1,1],[40.5,41.3],[-1.3,-0.8],
                [40,100],[10,100],[1,2],[0.3,0.6]]
    ...
    print("Correlation parameters are ",x)
    plt.figure()
...
filenames = ['Output/Bridget_mcmc_v1_output_W40']
filenames = ['Output/f1.5_Bridget_mcmc_v1_output_W40']
# this name gets added to all produced plots
prefix="Plots/f1.5_Bridget_v1_"
burnin=1000
main(filenames,labels,prefix,burnin=burnin,version=1)
exit()
