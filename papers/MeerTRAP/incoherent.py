...
def main():
    ...
    names=["MeerTRAPincoherent","MeerTRAPcoherent","DSA","CRAFT_ICS_1300"]
    ...
    point_labels = ["FRB 20240304B", None,None, None, None]

    # Set colours and styles for plotting contours and FRBs
    cmap = cmr.arctic
    data_clrs = cmap(np.linspace(0.0, 0.9, 5))
    temp = data_clrs[1].copy()
    data_clrs[1] = data_clrs[2]
    data_clrs[2] = temp
    cont_clrs = data_clrs
    markers=["*", "*","o", "o", "x"]
    markersize = [10, 10,4, 4, 5]
    ewidths = [1,1,1,1,1]
    styles=[":",":","-","-.","--"]
    ...
    misc_functions.plot_grid_2(g.rates,g.zvals,g.dmvals,
        name=opdir+name+"_zDM.png",norm=3,log=True,
        ...
        othernames = ["MeerKAT Incoherent","MeerKAT","DSA","CHIME","ASKAP"], 
    ...
    names = ["MeerTRAP incoherent","MeerTRAP coherent", "DSA 110", "ASKAP ICS"]
    styles=["-","--","-.",":"]
    ...
    for i,g in enumerate(gs):
        ...
        # Do plotting
        ax1.plot(g.zvals,pz,label=names[i],linestyle=styles[i],linewidth=2)
        ax2.plot(g.zvals,cpz,label=names[i],linestyle=styles[i],linewidth=2)
    ...
    ax1.plot(g.zvals,pz,label="CHIME",linestyle=":",linewidth=2)
    cpz = np.cumsum(pz)
    cpz /= cpz[-1]
    ax2.plot(g.zvals,cpz,label="CHIME",linestyle=":",linewidth=2)
    ...
    plt.legend(loc="lower right")
    plt.tight_layout()
    plt.savefig(opdir+"cum_pz_comparison.png")
    plt.close()
