...
def main():
    ...
    for i,state_dict in enumerate(state_dicts):
        ...
        surveys, grids = loading.surveys_and_grids(survey_names = names,\
                        repeaters=repeaters, sdir=sdir,nz=400,ndm=400,zmax=2,dmmax=2000,
    ...
    labels=["No width/scattering","CHIME Catalogue 1","lognormals (this work)","Best fit (this work)"]
    styles=[":","-.","--","-"]
    ...
    for i,r in enumerate(rates):
        ...
        ax1.plot(g.zvals,zdist/dz,label=labels[i],linestyle=styles[i])
        ax2.plot(g.dmvals,dmdist/ddm,label=labels[i],linestyle=styles[i])
...
def get_defined_states():
    ...
    state_dicts = [state_dict1, state_dict2, state_dict3, state_dict4]
    survey_dicts = [survey_dict1, survey_dict2, survey_dict3, survey_dict4]
