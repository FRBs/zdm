...
def main():
    ...
    exit()
    # plots six bins per
    #compare_rsets(Rset1,Rset2,prefix='DecbinFigs/nochime',DMcut=None,chime_response=False)
    
    #compare_rsets(Rset1,Rset2,prefix='DecbinFigs/chime',DMcut=None,chime_response=True)
...
def extreme_rsets(Rset1,Rset2,opdir='Extremes/',DMcut=None,chime_response=True,allplot=True,load=False,addshin=False):
    ...
    if allplot:
        ...
        plt.sca(ax)
        plt.text(0.54,0.91,text+', distributed',transform=ax.transAxes)
        ...
        plt.sca(ax4)
        plt.text(0.54,0.91,text+', strong',transform=ax4.transAxes)
        bins=np.linspace(0,4000,21)
        plt.hist(csingles,bins=bins,alpha=0.5,label='CHIME: single bursts',edgecolor='black')
        plt.hist(creps,bins=bins,alpha=0.5,label='repeating sources',edgecolor='black')
        leg1=plt.legend(loc='upper left', bbox_to_anchor=(0.4, 0.9))
        ...
        plt.sca(ax2)
        plt.text(0.5,0.3,text+', distributed',transform=ax2.transAxes)
        ...
        plt.sca(ax3)
        plt.text(0.5,0.3,text+', strong',transform=ax3.transAxes)
        plt.plot(dm,cchist,label='CHIME',color='red')
        plt.legend()
        plt.tight_layout()
...
def compare_rsets(Rset1,Rset2,pset=None,prefix="",Nbin=6,DMcut=None,chime_response=True):
    """
    Defined to test some corrections to the repeating FRBs method
    """
    # old implementation
