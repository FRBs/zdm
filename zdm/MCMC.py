    ...
    if in_priors == False:
        ...
        for s, grid in zip(surveys, grids):
            if s.nD==1:
                ...
                llsum2 = it.calc_likelihoods_2D(grid, s)
                llsum += llsum1 + llsum2
            else:
...
def mcmc_runner(logpf, params, surveys, grids, nwalkers=10, nburn=0, nsteps=100):
    ...
    sampler = emcee.EnsembleSampler(nwalkers, ndim, logpf, args=[params, surveys, grids])
    sampler.run_mcmc(starting_guesses, nsteps)

    posterior_sample = sampler.get_chain(discard=nburn)
