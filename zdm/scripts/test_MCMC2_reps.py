...
def main():
    ...
    survey_names = ["CHIME/CHIME_decbin_0_of_6", 
    ...
    surveys, grids = loading.surveys_and_grids(survey_names=survey_names, repeaters=True, init_state=state, alpha_method=1)

    state2 = parameters.State()
    state2.set_astropy_cosmo(Planck18)
    state2.update_params(param_dict)
    surveys2, grids2 = loading.surveys_and_grids(survey_names=survey_names, repeaters=True, init_state=state2, alpha_method=1)
